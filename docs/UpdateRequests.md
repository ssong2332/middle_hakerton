...
| 2 | 2026-08-04 | docs/Architecture.md | 설계 제1원칙 (line 27) | "코드 **0줄**, 태스크 **72건 전부 `todo`**(measured — `docs/Tasks.md` Status 열 grep, 2026-08-04)" — 이 문서 자신의 Last Updated(2026-08-04)와 같은 날짜지만 이후 커밋들과 더 이상 맞지 않음 | `git ls-files` 실측: `apps/web`·`apps/extension`·`packages/core`에 실제 소스 존재. `docs/Tasks.md` Status 열 실측(node 스크립트로 전수 집계): 총 72건 중 `done` 2건(T1, T2), `todo` 70건 — "코드 0줄·전부 todo"는 더 이상 사실이 아님 | architect | **resolved** (2026-08-05, architect 5차 패스 — `docs/Architecture.md` "설계 제1원칙" 절에 갱신 문단을 추가했다. 이 행이 적은 `done` 2건은 **작성 시점 값**이며, 처리 시점 재실측값은 **총 72건 · `done` 16 · `review` 2 · `todo` 54**다) |
| 3 | 2026-08-09 | docs/Architecture.md | Security "Attack surface" 행 (line 847) | "manifest 권한을 **`activeTab`** + 우리 API origin `host_permissions` + 콘텐츠 스크립트 `<all_urls>` 로 최소화하고 T58에서 1회 검토·기록(AC-054②)" — `activeTab`을 최소 권한 집합의 일부로 명시 | T58 구현 중 실제 권한 검토(AC-054②) 결과 — `activeTab`은 `chrome.tabs.*`/`chrome.action.*`/`chrome.scripting.*` 호출이 리포 전체에 0건이라(grep, 2026-08-09 measured) 아무 코드 경로도 이 권한을 쓰지 않는다. 이 확장은 클릭-주입형이 아니라 `content_scripts.matches: ["<all_urls>"]` 정적 선언으로 모든 사이트에 자동 주입되므로 `activeTab`이 부여하는 "사용자가 클릭한 탭에 한해 임시 접근" 능력을 애초에 쓸 방법이 없다 — T58에서 `manifest.json`에서 제거했다(`apps/extension/manifest.json`, PR #89). 실제 최소 집합은 `storage` + 빌드 시 주입되는 API origin `host_permissions` 1개 + `<all_urls>` 콘텐츠 스크립트뿐이다 | architect | open |

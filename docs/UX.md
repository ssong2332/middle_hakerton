...
| Based on PRD Version | v2.7 (2026-08-04) |
| Last Updated | 2026-08-04 |

**v4.0 summary** — This is a revalidation pass against docs/PRD.md v2.7 (previous pass was against v2.6, Document Version 3.0). PRD v2.7 reflects 11 user decisions and adds AC-059–AC-069 / MVP Scope rows #34–#37. This pass is additive-and-corrective, not a restructure: no screen is deprecated this round. Changes:
...
### Enrich Recipient Info via Public Profile URL (Flow ID: UF-018)
| Item | Value |
|---|---|
| Actor | Logged-in sender |
| Trigger | User clicks "상대방 정보 보강" (Enrich recipient info) next to the recipient identifier field on UX-004, typically for a recipient who is not (or may not be) a registered account |
| Related Acceptance Criteria | AC-065 |
| Steps | 1. User opens Recipient Public Profile Enrichment (UX-018) for the currently-entered recipient identifier. 2. User pastes **one** public GitHub profile URL into a single input field — nothing is searched, crawled, or auto-suggested (AC-065②). 3. User submits ("조회"). 4. System fetches that URL once and extracts only the raw `location`/`company` strings, if present (AC-065③) — no other field from the source is kept. 5. Screen shows the raw strings (or "미등록" per field if absent/unfetchable), plus the pasted source URL and the fetch timestamp (AC-065⑥). 6. If `location` yields a recognizable timezone, candidate timezone(s) are listed — none is applied yet. 7. User optionally selects and confirms one timezone candidate (AC-065④ — never auto-applied). 8. User saves. The confirmed values attach to that recipient and feed the next mediation run's personalization on top of whatever registered-account/pair-protocol data already applies. 9. User returns to UX-004. |
| Alternative Flow | User closes the screen without entering a URL, or without confirming a timezone — nothing is saved for the fields left blank; base mediation for that recipient is completely unaffected either way (AC-065①). User replaces a previously-saved URL with a new one — the new fetch overwrites the old values (never merges/accumulates from multiple URLs). |
| Failure Flow | Fetch fails (invalid URL, network error, rate limit) → inline error naming the failure; both fields show "미등록"; the source URL and attempt timestamp are still recorded so the user can see an attempt was made; user may retry the same URL or paste a different one. No value is ever guessed to fill the gap. |
| Success Criteria | `location`/`company` shown are always exactly what was fetched, or explicitly "미등록" — never invented (AC-065⑤, verified with ≥2 no-value cases); a confirmed timezone is never applied without the user's explicit selection (AC-065④); the source URL and fetch timestamp are visible whenever any value is populated (AC-065⑥); mediation for that recipient works identically whether or not this flow was ever used (AC-065①). |
...
| Acceptance Criteria | AC-001, AC-002, AC-003, AC-004, AC-005, AC-006, AC-007, AC-008, AC-009, AC-010, AC-015, AC-022, AC-029, AC-032, AC-041, AC-043, AC-046, AC-047, AC-056, AC-057, AC-058, AC-059, AC-061, AC-063 |
...
| Secondary Actions | Edit transformed text before approving; open Response Deadline Negotiation (UX-005) via the "Set response deadline" button OR directly from a Holiday Conflict warning (pre-fills the flagged deadline, AC-057, formerly AC-048③); open Scheduled Send (UX-006, NORMAL/LOW only); open Convert to Task Ticket (UX-007 — present only when AC-058's emotional-signal gate fires, absent otherwise, never disabled); open Recipient Public Profile Enrichment (UX-018) via a "상대방 정보 보강" link next to the recipient field (see Open Questions #12 on when this link should show); dismiss the emoji-risk warning. |
...
| Data Required | sender (current user), recipient identifier, message text, urgency override (optional), pair protocol for that recipient (if exists), sender's global profile (including honorific level — may be empty/skipped, AC-059), any recipient enrichment data from UX-018 (optional, if the user added it), project terminology dictionary (including person/honorific entries), prior diff history (personalization input only, not shown raw here), static holiday dataset (KR/US/**JP**/CN, 2026, hardcoded — Planning Decision #52, country list updated to AC-057/Planning Decision #74), emoji risk-tier lookup (높음/중간/낮음, no country field — AC-056①) |
...
| Item | Value |
|---|---|
| Belongs to Flow(s) | UF-015 (primary), UF-016 (Interpret mode, conditional P3), UF-019 (recipient candidate detection/selection, conditional P2), UF-011, UF-012, UF-014 (GitHub/Slack/Gmail — Insert path only, when the site is a registered Layer 2 module) |
| Acceptance Criteria | AC-052, AC-053, AC-010, AC-021, AC-040, AC-042, AC-051, AC-055 (conditional), AC-066, AC-061, AC-067 (conditional), AC-068 (conditional), AC-069 |
| Purpose | Let a user mediate (or, conditionally, interpret) any selected web text right where they are, without leaving the page — always via clipboard copy, and via direct insertion when the current site supports it. |
| User Goal | Get mediation help on whatever I'm reading or writing, wherever I am in the browser, without switching to another tab. |
...
| Navigation Targets | None (panel closes back into the host page); may deep-link to the web app's login when NotLoggedIn |
| Events Emitted | `selection_overlay_opened`, `overlay_mediation_requested`, `overlay_copied_to_clipboard`, `overlay_text_inserted`, `overlay_insertion_failed`, `overlay_interpret_requested` (conditional P3), `overlay_recipient_candidate_selected`, `overlay_recipient_candidate_cleared` (conditional P2) |
| Expected Outputs | Clipboard content (always-available path) or DOM-inserted text (conditional Layer 2 path); a diff/log record identical in shape to UX-004's; an interpretation result (conditional P3, never persisted as a message); a resolved recipient (optional, manual or detected) feeding the same personalization pipeline as UX-004 |
...
| Acceptance Criteria | AC-054, AC-068 (conditional content) |
| Purpose | Tell the user, once, what the extension reads and where it sends it before any Layer 1 mediation call can happen. |
| User Goal | Know what I'm agreeing to before I use this on a site full of my own or my company's text. |
| Entry | Automatic, on the extension's first activation after install — shown once, before any floating-button/panel interaction is reachable. |
...
| States | Shown: the one-time notice content — **with a conditional 4th disclosure item present only in a build where recipient-candidate detection (AC-067/T66) shipped** (see Business Rules) / Dismissed: notice closed, extension usable, will not reappear on this browser profile. There is no Loading/Error state — this is static local content with no network call. |
...
| Business Rules | Must state, at minimum: (a) only the text the user selects is sent, never the full page (AC-054①a); (b) the destination is our backend, which proxies OpenAI (AC-054①b, consistent with AC-030); (c) the extension holds a permission to operate on any site (AC-054①c). Copy must never claim a protection that isn't actually implemented (AC-054③, same principle as AC-034's "don't overstate what's built"). **Scope is confirmed fixed at notice-only** — no consent record is stored, no withdrawal/settings UI exists (PRD Open Question #23 is resolved as of Planning Decision #81; this document's prior "still open" framing is corrected). **Content is conditional on whether recipient-candidate detection (AC-067) shipped in this build (AC-068②③, PRD Planning Decision #83):** if T66 is `done` and shipped, this notice adds one item — "수신자 후보를 찾기 위해 현재 페이지의 일부를 브라우저 안에서 읽습니다. 그 내용은 전송·저장하지 않습니다." If T66 is cut/not shipped, that item is **not** shown. Whoever finalizes the build must check T66's status and match the notice text to it — showing an undone capability or hiding a shipped one both violate AC-054③/AC-068③. |
| Data Required | Static notice copy only (with one conditional item, above) — no user data is captured on this screen |
| Data Operations | None (no read/write beyond a local "already shown" flag) |
...
| Expected Outputs | A local "notice already shown" flag so it doesn't reappear; no server-side record (per the confirmed notice-only scope) |
| Assumptions | Extension is developer-mode-loaded only in MVP, so the audience is the team/judges installing it manually (Planning Decision #4) — not a general public install base. **This project deploys continuously to `main` before the 2026-08-21 submission (docs/PRD.md Delivery & Deployment)** — see Open Questions #13 on whether the "shown once" flag should be versioned to the notice's own content so a user who saw an earlier (pre-AC-067) build's notice doesn't keep an out-of-date understanding after a later merge changes what's actually read. |

### Recipient Public Profile Enrichment (Screen ID: UX-018)
| Item | Value |
|---|---|
| Belongs to Flow(s) | UF-018 |
| Acceptance Criteria | AC-065 |
| Purpose | Let a sender optionally strengthen personalization for a non-registered recipient using one public profile URL the sender pastes in — never an automated search. |
| User Goal | Give the tool a bit more real context about someone who hasn't signed up, without the tool going and digging on its own. |
| Entry | "상대방 정보 보강" link next to the recipient identifier field on UX-004 (see Open Questions #12 on whether this link should hide once the recipient is confirmed as a registered account). |
| Exit | "저장" (Save) → returns to UX-004, confirmed values applied to that recipient's personalization. "취소"/Close → returns to UX-004, nothing changed. |
| Primary Actions | Paste a GitHub public profile URL; Fetch ("조회"); Select/confirm a timezone candidate; Save. |
| Secondary Actions | Cancel/Close without saving; Clear the URL and remove any previously saved enrichment for this recipient (data-minimization control — this is the sender's own submitted data about the recipient, not the recipient's own account, so the sender who added it can also remove it; see UX Decision Log). |
| States | Empty: no URL entered yet / Loading: fetch in progress / Result: raw `location`/`company` strings shown exactly as fetched, or "미등록" per field when absent — plus the pasted source URL and fetch timestamp, always shown together with any populated value (AC-065⑥) / TimezoneCandidates: one or more candidate timezones listed for selection, none pre-selected / TimezoneUnconfirmed: candidates shown but none chosen yet — personalization does not use timezone until the user explicitly confirms one (AC-065④) / Error: fetch failed — explicit message, "미등록" shown for both fields, source URL/timestamp still recorded to show an attempt was made / Saved: confirmation, values now attached to this recipient. |
| Validation | URL field holds a single value at a time — pasting a new one replaces the old rather than appending or triggering any additional lookup (AC-065②). No format is enforced beyond "non-empty" — a bad/unreachable URL is reported through the Error state, not blocked at input time. "조회" enabled only when the field is non-empty. "저장" enabled once a fetch has completed (a result, or an explicit "미등록") — timezone confirmation is optional, Save works with location/company alone. |
| Failure | Fetch fails (invalid URL, network error, rate limit) → inline error naming the failure; "미등록" shown for both fields; user may retry the same URL or paste a different one; nothing is ever guessed to fill the gap (AC-065⑤). |
| Accessibility | URL input has an accessible label; timezone candidates form a keyboard-operable radio group, not click-only; "미등록," the source URL, and the fetch timestamp are all exposed as text, never icon/color alone. |
| Figma Frame | N/A — no Figma reference |

**Architect Handoff**
| Item | Value |
|---|---|
| Priority | Low (P2 feature, MVP Scope #34 — cut-eligible per Planning Decision #82⑤, and placed last in the P2 internal priority order per Planning Decision #92) |
| Business Rules | Fetch is triggered **exclusively** by the user's paste-and-submit action — no automatic search, crawl, or link-following of any kind exists in any code path (AC-065②). Only `location` and `company` are extracted and persisted; every other field the source might return (activity, repositories, posts, email, photo, etc.) is discarded, never stored (AC-065③). Timezone is never auto-applied — candidates are computed from the `location` string, but only take effect in personalization after the user's explicit confirmation (AC-065④). A value that cannot be obtained (no URL, fetch failure, empty field in the source) is shown as "미등록," never invented (AC-065⑤, same principle as AC-047②). The source URL and fetch timestamp are always shown once a fetch has been attempted (AC-065⑥). **This screen's copy must never use "분석했다"/profiling-style language about the recipient** — the recipient is a non-consenting third party who has no account and no awareness of this screen (PRD Risks, "제3자 개인정보 수집" row); copy must read as "we read the one page you gave us," never "we profiled this person." Mediation for any recipient works identically whether or not this screen was ever used (AC-065①) — this screen never gates or is required by UF-003. |
| Data Required | recipient identifier (from UX-004); pasted GitHub profile URL; fetched `location`/`company` raw strings (or their absence); selected timezone candidate (optional, user-confirmed); fetch timestamp |
| Data Operations | Create/Update (recipient enrichment record: URL, location, company, confirmed timezone, fetched-at timestamp); Delete (user-initiated clear); Read (on redisplay) |
| External Dependencies | GitHub public profile fetch (unauthenticated, one request per submit — feasibility/rate-limit **unverified**, docs/PRD.md Assumptions; T64's first-30-minute spike is the confirmation step, and the whole screen's fetch action is the only thing affected if that spike fails — Core is unaffected, P2) |
| Permissions | Requires authenticated session (the sender's); the recipient being enriched needs no account and is not notified — no consent mechanism exists for them in MVP (see PRD Risks) |
| Navigation Targets | Returns to UX-004 |
| Events Emitted | `recipient_profile_url_submitted`, `recipient_profile_fetched`, `recipient_profile_fetch_failed`, `recipient_timezone_confirmed`, `recipient_enrichment_saved`, `recipient_enrichment_cleared` |
| Expected Outputs | An enrichment record (location/company strings, confirmed timezone if any, source URL, fetched-at timestamp) that UF-003's personalization step reads for that recipient, layered on top of whatever registered-account/pair-protocol data already exists |
| Assumptions | GitHub allows unauthenticated fetch of public profile `location`/`company` fields — **unverified** (docs/PRD.md Assumptions; confirmed or refuted by T64's first-30-minute spike). `location` free text reliably maps to a timezone — **assumed false in general** (docs/PRD.md Assumptions: "Seoul"/"Tokyo, Japan" resolve, "Remote"/"Earth"/blank don't) — this is exactly why timezone is candidates-plus-confirmation rather than auto-applied; the design is correct even if this assumption is wrong. |
...
```
Design a small, one-time notice screen (extension-style panel or a lightweight first-run overlay, not a full webpage) shown the very first time a browser extension runs. It explains, in plain short text, three or four things: (1) only the text the user selects is ever sent anywhere — the extension does not read or store the rest of the page, (2) the destination is the product's own backend, which forwards to OpenAI, (3) the extension has permission to run on any website the user visits, and (4) CONDITIONALLY — only include this 4th item if recipient-candidate detection shipped in this build — "수신자 후보를 찾기 위해 현재 페이지의 일부를 브라우저 안에서 읽습니다. 그 내용은 전송·저장하지 않습니다." (we read part of the current page in-browser to find a recipient candidate; that content is never sent or stored). Design both a 3-item and a 4-item variant so implementer can use whichever matches the shipped build.
...
One state only: shown once, then dismissed (it will not reappear on this browser). No loading/error states — this is static local content with no network call.
...
### Recipient Public Profile Enrichment (covers: UX-018)
| Item | Value |
|---|---|
| Source Screens | UX-018 Recipient Public Profile Enrichment |
| Last Synced With | Document Version 4.0 |

```
Design a small modal (opens on top of the two-panel mediation workspace, similar scale to the deadline-negotiation modal) for optionally adding public context about a message recipient. A single input field for pasting one GitHub public profile URL, with a "조회" (Fetch) button.

Result area, once fetched: two labeled rows showing the raw values found — "회사 (Company)" and "지역 (Location)" — each either showing the exact text found or, if not found, the word "미등록" (Not on file). Directly below both rows, in smaller/secondary text, show the source: the pasted URL and "조회 시각: [timestamp]" (fetched at [timestamp]) — this source line should always be visible whenever any value is populated, so it never looks like the tool inherently knew this information.

If the location text could suggest a timezone, show a small list of 1-3 candidate timezones as a radio-button style choice, none pre-selected — the user must actively pick one. If they don't pick one, that's fine; only company/location save.

A "저장" (Save) primary button and a "취소" (Cancel) link/button. A small secondary "URL 지우기" (Clear) control that removes any previously saved enrichment for this recipient.

States: empty (no URL yet), loading (fetching), result (as described above), error (fetch failed — an inline message, with both fields showing "미등록" but the attempted URL/timestamp still shown), saved (brief confirmation).

Important tone constraint: nowhere in this screen's copy should it say anything like "we analyzed this person" or "profile analysis" — phrase everything as simply reading the one page the user provided (e.g., "붙여넣은 프로필에서 가져온 정보입니다" / "info from the profile you pasted"). This is about a third party who has no account here and didn't consent to being looked up, so the tone must stay factual and minimal, never analytical.

Keyboard-operable throughout; timezone candidates are a real radio group, not click-only.
```

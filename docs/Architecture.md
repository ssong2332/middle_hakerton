...
(2026-08-09 **9차 패스** — 🔴 **버전 헤더를 올리지 않았다, 이전 두 패스와 같은 이유로.** T35 리허설 중 발견된 "장면 4가 3회 중 2회 폴백됐다"(88초·48초 소요)는 finding을 재시도/타임아웃 설정 관점에서 분석했다. **결론: 코드 수정 대상이 아니다 — 프로덕션 경로(`openai.ts`)와 무관한, 로컬 전용 어댑터(`gemini.ts`)의 배치-테스트용 재시도 튜닝을 실시간 리허설에 그대로 쓴 것이 원인이다.** 근거(measured, `gemini.ts`·`openai.ts` 직접 열람): `openai.ts`는 `REQUEST_TIMEOUT_MS=3000`(3초) + `maxRetries:1`(SDK 기본 백오프만, **타임아웃 자체는 재시도 대상에서 명시적으로 제외** — 파일 자체 주석 "타임아웃 자체는 재시도하지 않는다, 같은 지연이 반복될 가능성이 높아 재시도가 소프트 타깃을 두 배로 깎아먹는다")로 PRD NFR "체감 5초"에 맞춰 타이트하게 설계되어 있다. 반면 `gemini.ts`는 `attempts:4, initialDelay:3, maxDelay:20`(파일 헤더 주석 "로컬 테스트 전용 — 정식 아키텍처 결정이 아니다")으로, **53건짜리 배치 회귀 스위트(`test:regression-c2`)가 Gemini 무료 티어 분당 상한을 견디도록** 튜닝된 값이다 — 상호작용형(1회 클릭) 리허설용이 아니다. 한 요청이 C1→C2→C4 최대 3단계를 순차 호출하므로, 각 단계가 이 배치용 최악값(단계당 최대 ~38~42초, jitter 포함)까지 갈 수 있어 88초·48초라는 관측치가 설명된다. **⚠️ 이것이 뜻하는 바**: 이번 발견은 프로덕션이 사용할 `openai.ts` 경로의 신뢰성에 대한 증거가 **아니다** — AC-026이 요구하는 "배포된 공개 URL + 실키" 경로는 여전히 실측되지 않은 채로 남아 있다(실키 자체가 없음, T35 기존 블로커 ⓒ). **재시도/타임아웃 코드를 바꾸지 않은 이유**: `gemini.ts`는 배치 회귀용으로 설계 근거가 이미 명시돼 있고(위 인용), 실시간 리허설에 맞게 값을 낮추면 그 배치 용도가 깨진다 — 두 가지 다른 용도에 하나의 설정을 억지로 맞추는 대신, **다음 상호작용형 리허설은 `openai.ts` 경로(실키 확보 후)로 하거나, `gemini.ts`를 쓸 경우 배치 튜닝이라는 한계를 미리 인지하고 진행**하는 것이 올바른 처리다. `docs/DemoScript.md`의 해당 각주(v2.8)는 이 문서를 인용해 "발표 당일 위험"이라는 과장된 프레이밍을 정정해야 한다(별도 조치, planner 소관).)
(2026-08-09 **8차 패스** — 🔴 **버전 헤더를 올리지 않았다, 7차 패스와 같은 이유로.** 이 패스는 **Tech Stack 표 1행만** 다뤘고 PRD/UX 나머지 절 재검증은 하지 않았다 — **버전 격차(PRD v3.2/UX 6.0 기준 vs 실제 v4.5/v6.4)는 이 패스로 줄어들지 않았고 여전히 해소가 필요하다(이월, 다음 architect 전체 패스의 안건).** 이 패스가 바꾼 것은 **Tech Stack 신규 행 1건**이다: **CSS 수단 — CSS Modules 유지, 신규 도구 도입 없음**(T76 라우팅, planner가 아니라 architect의 결정 — 새 사용자 게이트 아님, 기존 6개 게이트 항목과 무관). 근거: 이미 동작 중인 메커니즘(11개 CSS Module + `globals.css` 토큰)이 T76의 남은 작업(토큰 미적용 화면에 적용·하드코딩 리터럴 제거)을 전부 해결하며, 전처리기·유틸리티 프레임워크가 추가로 푸는 문제가 없다. `docs/UX.md` Design Tokens 절(v6.4)이 이미 카탈로그화한 토큰들의 단일 정의 위치(`globals.css`)를 재확인했을 뿐 옮기지 않는다.)
...
| CSS 수단 | **CSS Modules 유지(신규 도구 도입 없음)** — 디자인 토큰은 `apps/web/app/globals.css`의 CSS 커스텀 프로퍼티 1곳에만 정의(`docs/UX.md` Design Tokens 절, v6.4가 이름·값·용도를 카탈로그화) | 🔴 **(2026-08-09, T76 라우팅 — architect 결정, 신규 사용자 게이트 아님)** 이미 11개 CSS Module + `globals.css` 토큰이 동작 중이며(T12~T61 전 구간에서 검증됨), Tailwind는 애초에 이 표에 등재된 적이 없고 PRD/Tasks.md에만 언급되었을 뿐이다(Tasks.md T76 각주, planner measured). 전처리기(Sass 등)나 유틸리티 프레임워크(Tailwind 등) 도입은 **신규 의존성 + 빌드 설정 변경**을 요구하는데, 남은 작업(ⓐ미적용 화면에 기존 토큰 적용 ⓑ하드코딩 리터럴 제거)은 **이미 있는 메커니즘으로 전부 해결된다** — 새 도구가 푸는 문제가 없다. 토큰 단일 정의 위치는 이미 `globals.css`로 확정돼 있었고(T76 착수 전 실측), 이 결정은 그 위치를 재확인할 뿐 옮기지 않는다. |
